...
-- Defines a function which takes a "curried" function - i.e. one taking one arg at a time - and changes it into function a on a product
...
-- Define a function that glues some x ∈ X to a w ∈ Xⁿ
...
/-
We define three variables to be used in the following lemmas:
• ι acts as the index set (officially not a set) of...
• U, a cover (see hcover below) of Kⁿ where...
• K is a (compact) set in X.
-/
...
/-
Proves that for any x ∈ K, v ∈ Kⁿ, ∃ Uⱼ ∈ U s.t. (x,v) ∈ Uⱼ.
-/
...
/-
This lemma is where the bulk of the work for the main theorem of this file is done.
It proves that given that K, Kⁿ are compact, and U is an open cover of Kⁿ⁺¹, for any x ∈ K, ∃ a neighbourhood Nx of x s.t. Nx × K is finitely covered by U.
...
Notable Mathlib results used:
  ...
  • isOpen_pi_iff' :
      ...
      - The product of these open sets (an "open box neighbourhood") is a subset of S
      ...
      ...but in the interests of progressing elsewhere in the project, and given this section is already very bulky...
      ...it is left as yet unformalised.
-/

  intro x hx -- Let x ∈ K

  -- Apply previous lemma to get that (x,v) is in an element of U for every v ∈ Kⁿ
  have hglue_in_cover : ∀ v ∈ {v : Fin n → X | ∀ i, v i ∈ K}, ∃ j, glue x v ∈ U j :=
    glue_in_cover hcover x hx

  -- Define a choice function W using the above which takes a v ∈ K to a corresponding U i, or ∅ if v ∉ Kⁿ - requires classical reasoning
  classical
  let W := fun v : Fin n → X =>
  ...
  have hTV : ∀ v ∈ {v : Fin n → X | ∀ i, v i ∈ K}, ∃ Tᵥ, ∃ Vᵥ,
      ...
      -- Proves ∀ v ∈ Kⁿ, ∃ open neighbourhoods Tᵥ of x, Vᵥ of v s.t. Tᵥ × Vᵥ ⊆ W(v)
        ...
        -- Proves that the product of the last n co-ordinates of u is open and contains v
          ...
          constructor
          · rw [isOpen_pi_iff'] -- For the openness part, rewrite in terms of "box neighbhourhoods" - see preamble of lemma for discussion on this
  ...
  -- Now define a choice function taking a v ∈ K to Vᵥ as in hTV, and to ∅ if v ∉ Kⁿ
  let V := fun v : Fin n → X =>
  ...
  -- Proves there is a finite subcover of {V(v)}ᵥ covering Kⁿ, with indexes only beloning to Kⁿ
    ...
    -- Follows from induction hypothesis rewritten in terms of finite subcovers
      rw [isCompact_iff_finite_subcover] at hKn
      exact hKn V hV_open_cover.1 hV_open_cover.2
  ...
  obtain ⟨t, ht⟩ := hV_fin_cover -- Take t as witness to above - the index set of a finite subcover of Kⁿ by V's

  use ⋂ i ∈ t, T i -- Use as our neighbourhood of x, the intersection of T(i)'s over t
  constructor -- Break apart goal
  · constructor
    · -- First part: the intersection is open
      apply isOpen_biInter_finset -- Intersection is finite so suffices for each i ∈ t, T(i) open
      intro j hj -- Let j ∈ t
...
theorem IsCompact_pow_compact {K : Set X} (hK : IsCompact K) (hNonempty : K.Nonempty) :
  ...
  | succ n ih => -- Induction step : n+1
    ...
    have hnhd_prod_fin_cover : ∃ (t : Finset I), ⋃ i ∈ t₁, uncurry glue '' N i ×ˢ {v | ∀ (i : Fin n), v i ∈ K} ⊆
      ⋃ j ∈ t, U j := by
    -- Proves that the union over t₁ of N(i) × Kⁿ is covered by some finite subcover of U
    ...
    The following is essentially just a series of simple manipulations to get Lean to realise the same equivalence.
    -/
